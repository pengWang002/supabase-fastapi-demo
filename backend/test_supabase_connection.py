...
def load_env() -> Dict[str, str]:
    ...
    env = {k: os.getenv(k, "") for k in REQUIRED_VARS + OPTIONAL_VARS}
    missing = [k for k, v in env.items() if not v and k in REQUIRED_VARS]
    if missing:
...
def fetch_jwks(url: str, anon_key: str) -> Tuple[str, Dict]:
    ...
    for candidate in candidates:
        ...
        try:
            ...
            print(f"[jwks] probing {candidate} -> {resp.status_code}")
            resp.raise_for_status()
            jwks = resp.json()
            if jwks.get("keys"):
            ...
            continue
    raise RuntimeError(
        "JWKS endpoints not available. "
        "Supabase projects that use HS256 (default) do not expose JWKS; "
        "set SUPABASE_JWT_SECRET and verify tokens with HS256 instead. "
...
def main() -> int:
    ...
    if jwt_secret:
        print("[jwks] ⚠️  skipped (SUPABASE_JWT_SECRET provided, assuming HS256)")
    else:
        try:
